...
clear_cache() {
    print_color $BLUE "Clearing nginx cache..."
    echo
    
    # Common nginx cache locations
    CACHE_DIRS=(
        "/var/cache/nginx"
    ...
    # Also check for custom cache paths in nginx config
    CONFIG_PATHS=(
        "/etc/nginx/nginx.conf"
        "/usr/local/nginx/conf/nginx.conf"
        "/usr/local/etc/nginx/nginx.conf"
    ...
    # Look for custom cache paths in config files
    for config in "${CONFIG_PATHS[@]}"; do
        if [[ -f "$config" ]]; then
            # Extract cache paths from proxy_cache_path and fastcgi_cache_path directives
            while IFS= read -r line; do
                if [[ "$line" =~ proxy_cache_path[[:space:]]+([^[:space:]]+) ]] || [[ "$line" =~ fastcgi_cache_path[[:space:]]+([^[:space:]]+) ]]; then
                    cache_path="${BASH_REMATCH[1]}"
                    # Remove any trailing options after the path
                    cache_path=$(echo "$cache_path" | cut -d' ' -f1)
                    CUSTOM_CACHE_PATHS+=("$cache_path")
                fi
            done < <(grep -E "(proxy_cache_path|fastcgi_cache_path)" "$config" 2>/dev/null)
        fi
    done
    
    # Combine default and custom cache directories
    ALL_CACHE_DIRS=("${CACHE_DIRS[@]}" "${CUSTOM_CACHE_PATHS[@]}")
    
    CACHE_FOUND=false
    TOTAL_CLEARED=0
    
    for cache_dir in "${ALL_CACHE_DIRS[@]}"; do
        if [[ -d "$cache_dir" ]]; then
            CACHE_FOUND=true
            print_color $GREEN "Found cache directory: $cache_dir"
            ...
            echo "  Files to clear: $file_count"
            
            if [[ $file_count -gt 0 ]]; then
                read -p "  Clear this cache directory? (y/N): " confirm
                if [[ "$confirm" =~ ^[Yy]$ ]]; then
                    if sudo find "$cache_dir" -type f -delete 2>/dev/null; then
                        print_color $GREEN "  ✓ Cache cleared successfully"
                        TOTAL_CLEARED=$((TOTAL_CLEARED + file_count))
                    else
                        print_color $RED "  ✗ Failed to clear cache (permission denied?)"
                    fi
                else
                    print_color $YELLOW "  Skipped"
                fi
            else
                print_color $YELLOW "  Cache directory is already empty"
            fi
            echo
        fi
    done
    ...
    if [[ "$CACHE_FOUND" == false ]]; then
        print_color $YELLOW "No nginx cache directories found in common locations"
        echo
        print_color $BLUE "To manually clear cache, you can:"
        echo "1. Check your nginx configuration for custom cache paths"
        echo "2. Look for proxy_cache_path or fastcgi_cache_path directives"
        echo "3. Clear the specified cache directories manually"
        echo
        print_color $BLUE "Common cache configuration example:"
        echo "  proxy_cache_path /var/cache/nginx levels=1:2 keys_zone=my_cache:10m;"
    else
        if [[ $TOTAL_CLEARED -gt 0 ]]; then
            print_color $GREEN "Total files cleared: $TOTAL_CLEARED"
            print_color $YELLOW "Consider reloading nginx configuration to ensure proper cache operation"
        fi
    fi
...
show_menu() {
    ...
    echo
    print_color $GREEN "Cache Management:"
    echo "  14. Clear nginx cache"
    echo
    print_color $GREEN "Information:"
    echo "  15. Show nginx version"
    echo
